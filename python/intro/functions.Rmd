...
The `print()` function takes an input (in this example the input is the text "Hello, World!") and returns an output (namely, the same text it was given). So this isn't a very interesting function because it doesn't make any changes to its input, but it's nonetheless a very useful one. Another built-in function is `round()` which, by default, rounds a number off to the nearest whole number.

```{python}
round(123.456)
```

The above code rounds off the number 123.456 but then does nothing with it! We need to combine `round()` with `print()` if we want to see the result:

```{python}
print(round(123.456))
```

Alternatively, we can *assign* the output to a *variable* and print that variable instead of printing the output directly:
...
> For a list of all of Python's built-in functions as well as explanations about how they work, see the [Built-in Functions](https://docs.python.org/3/library/functions.html) documentation page
...
In this example the `round()` function was given *two* things: a numerical value and then the number of decimal places to which the value should be rounded-off. These 'things' are called *arguments* - the proper name for the inputs of a function.

> The inputs of a function are called **arguments** and the outputs are called **returns**. When you use a function it's known as **calling** it.

The second argument of `round()` actually has a name: `ndigits` (the **n**umber of **digits** to which you want to round off). You have the option of explicitly using this name when calling the function:

```{python}
result = round(123.456, ndigits=2)
print(result)
```

This is an example of a **keyword argument** (or a **kwarg** for short) - an argument that can be assigned to a keyword.
...
- Download and install Numpy from your computer's terminal with `pythonx.y -m pip install numpy` where `x.y` is the version of Python you have installed
    - If you don't know what version of Python you have installed, run `python --version` on your terminal. If this returns a version number that starts with a 2, it means that Python version 2 is your default version. However, chances are that you also have version 3 installed, so run `python3 --version` to see if that works and use that version number instead.
        ...
        - For example, if `python --version` returns `Python 2.7.18` then you should run `python3 --version`. If this returns `Python 3.13.5` then it means you have Python 3.13 installed and should use `python3.13 -m pip install numpy` to install Numpy.
...
- Giving it a name
- Listing the **parameters** (inputs) your function requires inside round brackets
...
-----------------------
**Parameters** are the placeholder names of the inputs to a function while the **arguments** are the actual values that get used when a a function is called. Have a look at the following example:

```{python}
def double_a_number(number):  # `number` is a parameter
    print(2 * number)


double_a_number(32)  # `32` is an argument
```

In this example, `number` is a parameter as it is the placeholder name for an input to the function; if appears when the function is *defined*. The value `32` is an argument as it is the actual value that is passed into the function, it appears when the function is *called*.
...
A single asterisk used like this is known as the *unpacking operator*.
...
In addition to the entire argument being optional, the keyword on its own is optional:

```{python}
# Call the function implicitly indicating num_sig_figs
rounded = sigfigs(1.2345, 3)
print(rounded)
```

Note that the phrase "keyword argument" implies that a keyword is being used; in the above example the second argument is not actually a keyword argument, it's an optional positional argument.

> When a function argument is passed using a *keyword* it is referred to as a *keyword argument*
...
Importantly, when using keyword arguments *the order doesn't matter*! This is what separates them from positional arguments where the order does matter (by definition - that's what "positional" means!).
...
Watch out for default arguments that are *mutable* (changeable), it's best practice to avoid them entirely! Take the following example of a function that can be used to add the result of a sports team's latest match to a list of their results for the season so far:

```{python}
def add_result(latest, season=[]):
    season.append(latest)
    return season
```

The default value for `season` is an empty list, which is mutable. If their first game is a win we can use the function as follows:

```{python}
latest_result = 'W'
results_this_season = add_result(latest_result)
print(results_this_season)
```

So far so good, it's the first match of the season so the fact that the default value for their previous results --- an empty list -- was used is appropriate!

```{python}
latest_result = 'L'
results_this_season = add_result(latest_result, results_this_season)
print(results_this_season)
```

After their second match things are working as expects, their results are `['W', 'L']`. However, if their season ends their and we try to start a new season of results we get the following:
...
That's unexpected! The default value for the previous results is meant to be an empty list but we are still seeing last season's results. The reason for this is that Python only records the default values for parameters once -- when the function is defined -- *not* every time the function is called. This means that if we augment a mutable default value within a function this change will persist once we leave the function scope (ie the changes will be kept).
