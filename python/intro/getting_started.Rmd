...
This creates the requirements.txt file using *only the packages that are actually used* by whatever Python files are in the folder your terminal is pointing at. However, while pipreqs worked immediately for me on my macOS machine, on my Ubuntu machine it wouldn't run in my terminal (even when I put it in my PATH). Not only that but running it with Python gave me the following error:
...
...which is caused by running a Python script that tries to import a package with the same name as the file itself. I got around this by locating the file (it was in `/usr/local/lib/python3.9/dist-packages/pipreqs`) and re-naming it to `python_pipreqs.py` after which it worked from the terminal with:
