...
#### **Configuring Python**
If you followed Option 2 above and downloaded and installed Python 'manually' (ie from the Python website) you had the option to configure the installation when you ran `configure`:

```bash
$ ./configure <options>
```

If you used Option 1 (ie if you used `apt-get`) then you bypassed this opportunity. For many users this won't matter. However, there are a couple of use cases where Python needs to be configured so as to have a **shared library** - one example is if you want to use Python in R Markdown using R's "reticulate" package. In general, if you come across the recommendation to build Python using `./configure --enable-shared` then this is the sign you want a shared library (because this is what that step creates). Fortunately we can create one manually, using one line of code:

```bash
$ sudo apt-get install libpython3.12
```

This will download the `libpython3.12` Ubuntu package which is the shared Python runtime library for Python 3.12 (for the record, this package's homepage is over [here](https://packages.ubuntu.com/mantic/libpython3.12)).
