...
We got a result of `False` because Python was searching for a *list* inside the list `ls` instead of searching for two separate values (and it didn't find that list). However, if we convert to sets we can use the `issubset()` method which does the job in one line:
...
---------------------------------
Sets have the `.add()` method which allows you to **add** elements:
...
They also have the `.update()` method which allows you to add multiple elements at a time:
...
They also have two methods which allow you to remove elements:

- `.remove()` will remove an element from the set if the element is indeed in the set - it will raise an error if the element is not there
- `.discard()` will remove an element from the set if the element is indeed in the set and will do nothing (not raise an error) if the element is not there
...
Tuples
======
Tuples are similar to lists and sets in that they contain mul**tuple** objects but, again, they are different to both:

- A tuple is **immutable** (unlike sets and lists which are *mutable*) which means that it *cannot be changed once created*
...
| Set            | `set()`           | `{}`            | True    | True   | False   | False         |

Tuples are slightly more memory- and time-efficient to use than lists but are less flexible due to being immutable.
