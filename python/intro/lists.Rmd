...
Notice that this **does _not_ preserve order**: we would have expected the result to be `['First', 'Second', 'Third']` but instead it was `['Second', 'First', 'Third']`. The easiest way to get the unique values in a list *in the same order* they appear in that list is to use a Pandas Series object, see [here](../advanced/data_frames_searching.html#bonus-removing-duplicates-from-a-list-whilst-preserving-the-order).
