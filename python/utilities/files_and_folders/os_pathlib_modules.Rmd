...
Files can't be created with `os` or `pathlib` but they can be created in base Python:

```{python, results = 'hide'}
# Create files
paths = [
    os.path.join('Example Directory - os', 'Example File 1.txt'),
    os.path.join(path_os, 'Example File 2.txt'),
    Path('Example Directory - pathlib', 'Example File 1.txt'),
...
Another Option
--------------
The `os.system()` function runs a piece of terminal/command line code. For example, the `ls` command **l**i**s**ts the files and folders within a folder:

```{python, eval = FALSE}
print(os.system('ls "Example Directory - os"'))
```
...
for dirpath, dirnames, filenames in os.walk('Example Directory - os'):
    # Sort the dirnames to ensure you search them in order
