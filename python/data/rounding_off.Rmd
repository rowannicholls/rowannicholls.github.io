...
def round_to_sig_figs(number, num_sig_figs):
    ...
    # Remove trailing zero if one exists and is not necessary
    if (
        (len(output.replace('.', '')) > num_sig_figs) and
        (output.endswith('.0'))
    ):
        if rounded.is_integer():
            output = str(int(rounded))

    # Add trailing zeroes if necessary
    if len(output.replace('.', '').lstrip('-').lstrip('0')) < num_sig_figs:
        current_sig_figs = len(output.replace('.', '').lstrip('-').lstrip('0'))
        discrepancy = num_sig_figs - current_sig_figs
