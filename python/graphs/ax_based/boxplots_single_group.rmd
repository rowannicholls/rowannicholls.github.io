...
But it is often easier to use a Pandas data frame. Note, however, that you do need to transpose the data frame (which is done with the `.T` method):
...
The plotting itself is going to be done in two lines: one to create the plot and the axes:  
`ax = plt.axes()`  
...
bp = ax.boxplot(data.T, sym='', whis='range', widths=0.4, labels=list(data))
# AXIS DETAILS
ax.set_title("Men's Long Jump Finals")
ax.set_ylabel('Distance [m]')
ax.set_xlabel('Olympics')
...
xticks = ax.set_xticks(np.arange(len(data.T) + 1) + 0.5, minor=True)
plt.show()
```
...
ax = plt.axes()
bp = ax.boxplot(
    data.T, sym='', whis='range', widths=0.4, labels=list(data),
    patch_artist=True
)
# AXIS DETAILS
ax.set_title("Men's Long Jump Finals")
...
medians = [bp['medians'][i].get_ydata()[0] for i in range(len(data.T))]
medians = [str(round(s, 2)) for s in medians]
# GET THE X-POSITIONS FOR THE LABELS
x_pos = np.arange(len(data.T)) + 1
...
=====================
A background can help make the plot look more professional and also aid in interpretation if gridlines are used.
...
def plot_boxplots(data, ax, title):
    #
    # PLOT
    #
    bp = ax.boxplot(
...
# COLLATE
data = [
    long_jump_men, long_jump_women, shot_put_men,
    shot_put_women, javelin_men, javelin_women
]
titles = [
...
```{python, fig.height=11.7, fig.width=8.8}
# CALL FUNCTION
...
for i, ax in enumerate(axes.flat):
    plot_boxplots(data[i], ax, titles[i])
plt.show()
```
