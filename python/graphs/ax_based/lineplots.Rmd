...
- **Transparency of the line**: use the `alpha` keyword argument within `plt.plot()`
...
This is obviously quite obtuse, so a simpler option would be to use `plt.fill_between()` which fills in the area *between* lines with colour. If you only specify one line then it will default to assuming that the second line is the x-axis:
...
- In order to create enough space for all of these plots, it's a good idea to re-size your figure. This topic is discussed on it's own [separate page](../image_size.html) but, in short, your options are:
    ...
    - `plt.rc('figure', figsize=(w, h))` set the same figsize parameter as above but for *all* the figures in your code
...
--------------------
See [here](../latex_in_labels.html) for more about adding Latex formatting in the title and axes' labels and see [here](../image_size.html) for more about changing the image size.
...
ax.set_ylabel(r'Output, $L$ [\textlambda]')
ax.set_ylim(0, 100)
