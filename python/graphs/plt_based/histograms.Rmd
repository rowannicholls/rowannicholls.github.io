---
title: '<font size="5">Graphs in Python:</font><br>Histograms<br><font size="5">_without using axes objects_</font>'
output:
    html_document:
        theme: paper
        highlight: textmate
...
- Although histograms and bar plots look similar, they are not the same and you should use different functions in Python to draw them (`plt.bar()` and `plt.hist()`, respectively)
...
We can change the number of bins to make the histogram more or less precise by adding a second parameter to the `plt.hist()` function call (or by using its keyword: `bins`):
...
Having *more* bins will make each one finer and will *increase* the precision while having *fewer* bins will make each one coarser and will *lower* the precision.
