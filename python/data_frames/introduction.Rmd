---
title: '<font size="5">Data Frames with `Pandas`</font><br>Introduction'
output:
    html_document:
        theme: paper
        highlight: textmate
...
> 
> 1. Introduction
> 2. [Searching, Finding, Filtering](searching.html)
> 3. [Calculations](calculations.html)
> 4. [Multiple Levels](multiple_levels.html)
> 5. [Dropping Empty Columns](dropping.html)
> 6. [Merging](merging.html)
> 7. [Grouping and Aggregating](grouping_aggregating.html)
> 8. [Pivot Tables](pivot_tables.html)
...
df = pd.read_csv('introduction/data frame.csv')
```
...
```{python}
# Import comma-separated values (.csv file) as a data frame
data_url = 'https://raw.githubusercontent.com/rowannicholls/rowannicholls.github.io/master/python/data_frames/introduction/data%20frame.csv'
...
Row-by-row
----------
The length of a data frame as given by `len(df)` is equal to the number of rows. This can be used to iterate over the rows in a for-loop:

```{python}
for i in range(len(df)):
    print(i, df.loc[i, 'Athlete'])
```

A data frame's rows can also be iterated over using `.iterrows()` which returns the rows' indexes as integers and the rows' contents as series:
...
Alternatively, the rows' contents can be returned as tuples using the `.itertuples()` method:
...
for row in df.itertuples():
    ...
    athlete_age = row.Age
    print(row_number, athlete_name, athlete_age)
```

Column-by-column
----------------
When using a for-loop to iterate over a data frame, it will iterate over the *column names:*
...
Using the `list()` function on a data frame will return its column names. This can then be used in a near-identical way to the previous example to iterate over the columns via a for-loop:

```{python}
for colname in list(df):
    print(colname, df[colname][1])
```

Iterate over a data frame's column names *and* column contents using the `.items()` method:
...
df.to_csv('introduction/data frame.csv', index=False)
```
