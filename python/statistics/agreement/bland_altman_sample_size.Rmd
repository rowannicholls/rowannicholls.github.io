...
This page complements [this one](bland_altman.html) on Bland-Altman analysis.
...
The *level of agreement* - the distance of the limits of agreement from the mean difference - will be set, as mentioned above, at a distance from the mean such that 95% of the sample data will fall within the LOAs:
...
Now the confidence intervals are calculated. The *level of confidence* - the width of the confidence intervals - will be taken as 99%, as discussed above, so that we can be 99% sure that the LOAs of the full study will fall within them:
...
- $\delta$ is the acceptance criterion (AC)
- ${LOA}_{upper}$ and ${LOA}_{lower}$ are the two limits of agreement as calculated above
- $t^*$ is the critical *t*-statistic and ${se}_{LOAs}$ is the standard error of the LOAs for a given sample size

This is a little tricky to calculate because both $t^*$ and ${se}_{LOAs}$ depend on $n$. We could use an optimisation algorithm to find a solution for it but, instead, we will just loop through different potential values, check each one and stop when we reach the point where the AC is met:
...
Both -0.001666 and 0.003999 are less than the AC (0.004) in magnitude, so we can be 99% sure that the full study will pass if we use this sample size (**if** the pilot study was representative).
...
warnings.filterwarnings('ignore', category=RuntimeWarning, message='divide by zero encountered in _nct_cdf')
warnings.filterwarnings('ignore', category=RuntimeWarning, message='invalid value encountered in _nct_cdf')
```
...
<!-- https://github.com/arcaldwell49/SimplyAgree/issues/44 -->

### Method 3: `blandPowerCurve()` from SimplyAgree in R
Importing the SimplyAgree library in R gives us access to the `blandPowerCurve()` function (see the [documentation](https://aaroncaldwell.us/SimplyAgree/reference/blandPowerCurve.html) and the [source code](https://github.com/arcaldwell49/SimplyAgree/blob/master/R/blandPowerCurve.R)). Let's use that to run the same example from Section 5 of Lu et al:
...
print(f'A sample size of {n} will ensure that the test has a statistical power of {round(p, 3)}')
## A sample size of 145 will ensure that the test has a statistical power of 0.8
```

### Aside: Calculating Bias and Standard Deviation from LOAs
Sometimes you'll find a data source that reports the LOAs but not the mean difference, $\bar d$ (the bias) or the sample standard deviation, $s$. That's not a problem, though, because you can work these out by considering the definitions of the LOAs:
