...
> [1] Bland JM, Altman DG. Statistical methods for assessing agreement between two methods of clinical measurement. Lancet. 1986 Feb;327(8476):307–10. DOI: [10.1016/S0140-6736(86)90837-8](https://linkinghub.elsevier.com/retrieve/pii/S0140673686908378). PMID: [2868172](https://pubmed.ncbi.nlm.nih.gov/2868172/). Available [here](https://www-users.york.ac.uk/~mb55/meas//ba.pdf).
...
=============================
As Bland and Altman say in their paper, at this point researchers will often calculate the [Pearson correlation coefficient](correlation_coefficients.html) ($r$) between the two methods. Doing so will give $r = 0.94$ with $p < 0.001$:
...
- The Pearson correlation coefficient $r$ measures how *correlated* two variables are, not the extent to which they *agree*. Perfect agreement implies that all the points lie along the line of equality, while a large $r$ value merely implies that they all lie on *a* straight line (which could be *any* straight line)
...
=====================
In essence, if we are interested in knowing to what extent two measurement methods differ we should calculate the average difference between the values they produce when measuring the same participant. If this is small it means that it is effectively irrelevant which method you use; both will yield a similar result. The two methods or devices can thus be used interchangeably, or one can be used instead of the other if it is preferable for whatever reason. To check that the measurement differences are not related to the actual values that are being measured, it is useful to plot them against the means of the pairs of values (ideally you would plot them against the true values, but as these are unknown the means of the pairs of values that were measured are your best guesses).
...
print(f'For the differences, x̄ = {bias:.2f} L/min and s = {s:.2f} L/min')
```
...
*\*Assuming a normal distribution for the difference is reasonable here because, even if the raw measurements themselves are not normally distributed, by taking the difference between pairs of measurements we remove a lot of the variation and probably end up with mostly measurement error.*

Aside: Values for the Z-Scores
------------------------------
The number of standard deviations away from the mean (the z-score) that corresponds to the endpoints of an interval that contains the central 95% of the standard normal distribution is approximately equal to 1.96. This value is used in the calculation above and yield LOAs of 73.86 L/min and -78.10 L/min. Bland and Altman used a rounded value of 2 in their paper (and also used a rounded value of 38.8 for the sample standard deviation) and thus got the slightly less accurate answers of 75.5 L/min and -79.7 L/min. For maximum precision, we can calculate the z-score for a confidence level of 95% directly:
...
$${se}_{LOA} = \sqrt{\frac{3{s}^2}{n}}$$

These standard errors can be converted into 'confidence intervals', for example 95% confidence intervals in which we can say with 95% confidence that the true values of the bias and of the LOAs lie. If we use the Student's *t*-distribution\*, the formula for the endpoints of a confidence interval is:
...
print(
    f'Lower LOA = {np.round(lower_loa, 2)} {np.round(ci_lower_loa, 2)};',
    f'Bias = {np.round(bias, 2)} {np.round(ci_bias, 2)};',
    f'Upper LOA = {np.round(upper_loa, 2)} {np.round(ci_upper_loa, 2)}'
)
```
...
- The sample size is small ($n = 17$)
    + We are therefore using the __*t*-distribution__ to calculate the confidence intervals. If the sample size was large ($n ≥ 30$) we could use the normal distribution to calculate the confidence intervals as well, see [the page on confidence intervals](../confidence_intervals.html) for more.
...
$${CI}_{bias} = 2 \times t^* \times \sqrt{\dfrac{ {s}^2}{n}}$$

$${CI}_{LOAs} = 2 \times t^* \times \sqrt{\dfrac{3 {s}^2}{n}}$$

so the widths of the confidence intervals *relative to the sample standard deviations* are:

$${CI}_{bias} = 2 \times t^* \times \sqrt{\dfrac{1}{n}}$$

$${CI}_{LOA} = 2 \times t^* \times \sqrt{\dfrac{3}{n}}$$
