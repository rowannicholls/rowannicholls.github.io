...
```
python3.9 -m pip install pingouin
```
...
Interpretation
==============
Again we can look at the Wikipedia page for help, as it gives this guide for interpreting the ICC (re-produced from Cicchetti (1994)<sup>[1]</sup>):
...
...plus this alternative one from Koo and Li (2016)<sup>[2]</sup>:
...
<sup>[1]</sup>Cicchetti D. Guidelines, criteria, and rules of thumb for evaluating normed and standardized assessment instruments in psychology. Psychological Assessment. 1994;6(4):284-290  
<sup>[2]</sup>2. Koo T, Li M. A Guideline of Selecting and Reporting Intraclass Correlation Coefficients for Reliability Research. Journal of Chiropractic Medicine. 2016;15(2):155-163
...
```{python}
agreement = interpret_icc_cicchetti(icc.loc['Single random raters', 'ICC'])

print(f"The inter-rate agreement ({icc.loc['Single random raters', 'ICC'].round(3)}) is {agreement}")
```

```{python}
agreement = interpret_icc_koo_li(icc.loc['Single random raters', 'ICC'])
