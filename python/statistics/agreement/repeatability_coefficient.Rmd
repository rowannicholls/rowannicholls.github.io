...
def repeatability_analysis(df):
    """Calculate agreement statistics."""
    columns = list(df)
    df['Sample Standard Deviation'] = df[columns].std(axis=1, ddof=1)
    df['Sample Variance'] = df['Sample Standard Deviation']**2

    # Create a summary
    summary = pd.DataFrame()
    # Mean
    means = ['Mean of ' + test for test in columns]
    for i, mean in enumerate(means):
        summary.loc[1, mean] = df[columns[i]].mean()
    # Sample size
    summary.loc[1, 'N'] = df.shape[0]
    # Degrees of freedom
    summary.loc[1, 'DoF'] = df.shape[0] - 1
    # Mean variance
    summary.loc[1, 'Mean Variance'] = df['Sample Variance'].mean()
    # Within-subject standard deviation
    s_w = np.sqrt(df['Sample Variance'].mean())
...
# Calculate agreement statistics
df, summary = repeatability_analysis(df)
print(summary)
```
...
j = pd.DataFrame({
    'Measurement 1': [
        100, 108, 76, 108, 124, 122, 116, 114, 100, 108, 100, 108, 112, 104, 106, 122, 100, 118, 140, 150, 166, 148, 174,
        174, 140, 128, 146, 146, 220, 208, 94, 114, 126, 124, 110, 90, 106, 218, 130, 136, 100, 100, 124, 164, 100, 136,
        114, 148, 160, 84, 156, 110, 100, 100, 86, 106, 108, 168, 166, 146, 204, 96, 134, 138, 134, 156, 124, 114, 112,
        112, 202, 132, 158, 88, 170, 182, 112, 120, 110, 112, 154, 116, 108, 106, 122
    ],
    'Measurement 2': [
    ...
    'Measurement 3': [
        107, 108, 82, 104, 112, 124, 102, 112, 112, 100, 104, 122, 110, 104, 102, 114, 102, 120, 138, 144, 154, 134, 166,
        150, 144, 130, 140, 148, 220, 192, 86, 116, 122, 132, 128, 94, 110, 208, 130, 130, 88, 88, 122, 154, 100, 122,
        122, 132, 148, 98, 152, 98, 106, 94, 76, 110, 106, 178, 154, 132, 188, 86, 124, 140, 142, 154, 138, 114, 122, 134,
        228, 134, 152, 88, 176, 180, 124, 120, 106, 106, 130, 94, 114, 100, 112
    ],
})

# Calculate agreement statistics
j, summary = repeatability_analysis(j)
print(summary)
```
...
s = pd.DataFrame({
    'Measurement 1': [
        122, 121, 95, 127, 140, 139, 122, 130, 119, 126, 107, 123, 131, 123, 127, 142, 104, 117, 139, 143, 181, 149, 173,
        160, 158, 139, 153, 138, 228, 190, 103, 131, 131, 126, 121, 97, 116, 215, 141, 153, 113, 109, 145, 192, 112, 152,
        141, 206, 151, 112, 162, 117, 119, 136, 112, 120, 117, 194, 167, 173, 228, 77, 154, 154, 145, 200, 188, 149, 136,
        128, 204, 184, 163, 93, 178, 202, 162, 227, 133, 202, 158, 124, 114, 137, 121
    ],
    'Measurement 2': [
        128, 127, 94, 127, 131, 142, 112, 129, 122, 113, 113, 125, 129, 126, 119, 133, 116, 113, 127, 155, 170, 156, 170,
        155, 152, 144, 150, 144, 228, 183, 99, 131, 123, 129, 114, 94, 121, 201, 133, 143, 107, 105, 102, 178, 116, 144,
        141, 188, 147, 125, 165, 118, 131, 116, 115, 118, 118, 191, 160, 161, 218, 89, 156, 155, 154, 180, 147, 217, 132,
        125, 222, 187, 160, 88, 181, 199, 166, 227, 127, 190, 121, 149, 118, 135, 123
    ],
    'Measurement 3': [
        124, 128, 98, 135, 124, 136, 112, 135, 122, 111, 111, 125, 122, 114, 126, 137, 115, 112, 113, 133, 166, 140, 154,
        170, 154, 141, 154, 131, 226, 184, 106, 124, 124, 125, 125, 96, 127, 207, 146, 138, 102, 97, 137, 171, 116, 147,
        137, 166, 136, 124, 189, 109, 124, 113, 104, 132, 115, 196, 161, 154, 189, 101, 141, 148, 166, 179, 139, 192, 133,
        142, 224, 192, 152, 88, 181, 195, 148, 219, 126, 213, 134, 137, 126, 134, 128
    ],
})

# Calculate agreement statistics
s, summary = repeatability_analysis(s)
print(summary)
```
