...
Example Data
============
This page will use fake data for its examples. Creating this will use three modules: `random`, which comes pre-installed with Python, and then `pandas` and `mimesis` which can be downloaded and installed from the terminal with the following commands (replace `3.11` with the version of Python you have installed):
...
Once installed, the fake data can be created in Python as follows:
...
# Labels
ax.set_title('Displaying Nominal Data in a Bar Plor')
ax.set_ylabel('Count')
...
# Labels
ax.set_title('Displaying Binary and Ordinal Data in a Bar Plor')
ax.set_ylabel('Count')
...
tails = 2
# Number of standard deviations
z_critical = stats.norm.ppf(1 - alpha / tails)
