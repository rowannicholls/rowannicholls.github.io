---
title: '<font size="5">Statistics in Python:</font><br>Paired Two One-Sided t-Test (TOST)'
output:
    html_document:
        theme: paper
        highlight: textmate
...
> This page on the *paired* two one-sided t-test ('paired TOST') is a follow-on from [this one](tost_unpaired.html) which introduces the *unpaired* two one-sided t-test ('unpaired TOST')

The Paired TOST
===============
We'll use the same example data that was used on the unpaired TOST page. It comes from [the documentation](https://pingouin-stats.org/generated/pingouin.tost.html) for Pingouin's `tost()` function.
