...
The code on this page uses the `sklearn`, `pandas`, `numpy`, `matplotlib` and `scipy` packages. These can be installed from the terminal with:
...
- $H_0:$ true mean house price is the same for both location types
- $H_1:$ true mean house price is **not** the same for both location types

The null hypothesis is that there is equality and the alternate hypothesis is that there is a difference (of any size). Note that if our research question was instead "are the mean house prices *equivalent*?" or "are the mean house prices the same to within a certain *tolerance*?" then the hypotheses above would not be formulated correct. Examples that have research questions like these are over [here](tost_unpaired.html).
                    ...
                    cd_number [label='Number of\ngroups/samples\nfor the IV?', fillcolor='#c9daf8']
                        two_parametric [label='Parametric?', fillcolor='#c9daf8']
                            true_independent [label='Independent\ngroups/sample?', fillcolor='#c9daf8']
                                'Unpaired\ntwo-sample\nt-test' [fillcolor='#d9ead3', label=<Unpaired<BR/>two-sample<BR/><I>t</I>-test>]
...
-----------------------
This *p*-value is small (<0.05), which means that we can reject $H_0$ and conclude that there is a statistically significant difference between the two location types (*p* < 0.05) with the median house price being $2,990 higher on average in areas bounded by the Charles river than in areas away from the river.
