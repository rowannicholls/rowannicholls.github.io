...
- Use the `time` module for time stamping, ie if you care about when something happened *relative to other things*, not the actual calendar date or clock time when it happened. This module is discussed on its [own page](../intro/date_time.html).
...
However, there is now also the option to use the `strptime()` (**str**ing **p**arse **time**) function to create a datetime object from a string in a *custom format*. You can define this custom format using the [format codes](https://docs.python.org/3/library/datetime.html#strftime-and-strptime-format-codes):
