...
- **Ubuntu**: you actually don't need to do anything because Git comes pre-installed (but you can upgrade to the latest version if you don't already have it with `sudo apt-get install git`)
- **macOS**:
...
Double check that you have Git installed (and see what version you have at the same time) by running `git --version` from the terminal. This should return the version number, eg `git version 2.25.1`. See the full documentation for installing Git [here](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git).
