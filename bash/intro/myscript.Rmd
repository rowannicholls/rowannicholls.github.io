---
title: '<font size="5">Introduction to Bash:</font><br>Creating and Running a Bash Script'
output:
    html_document:
        theme: paper
        highlight: textmate
...
__*Bash*__ is the default language used to run commands on the Terminal.

- The __*Terminal*__ is the default command-line interpreter (CLI) for macOS and most Linux computers, including Ubuntu
- Bash is a __*command language*__ which means that it is useful for running other programmes and, therefore, for tasks such as _automation_ and _batch processing_
    - This is where it differs from higher-level languages such as Python, R and MATLAB: while those languages are useful for _creating_ scripts that function as standalone programmes, Bash is useful for _running_ those scripts when there are many of them and when the results of one need to be communicated to another
- This means that, if you want to start writing advanced sets of scripts on a macOS or Linux machine, chances are you'll want to learn the basics of Bash
- In addition to being a language, Bash is a _shell_. A 'shell' is any user interface that can be used to run an operating system's functions. A shell can use a CLI (which is what Bash does when it runs on the Terminal) or a graphical user interface (GUI).

Bash stands for "Bourne-again shell", a double pun that derives both from the name of its inventor (Stephen Bourne) and the fact that it is a 'born-again' version of an older shell.
...
> __*Note 1:*__ while there is a convention to use the `.sh` extension for Bash scripts, it is not strictly necessary and can be omitted. It can be useful, however: if you are using an IDE such as Sublime Text to edit and run the script it will let it know what type of file it is, helping with syntax highlighting and automatic building. It can also help you control what happens when the file is double-clicked (see below).
...
=====================
There are a number of ways to do this (if the script runs successfully, you will see the words "Hello, World" because that is what your programme 'myscript' does!):
