---
title: '<font size="5">Introduction to Bash:</font><br>Variables and Data Types'
output:
    html_document:
        theme: paper
        highlight: textmate
...
Variables
=========
Assign a value to a variable using a single equals sign and no spaces, then use the `echo` function to display the value of the variable. When you do the call, you need to prepend a dollar sign; that's Bash's way of knowing you're using a variable:
...
- Calling a variable with **no quotation marks** means each word is treated as a separate thing. Bash doesn't know how to handle spaces, tabs and newlines by default and thinks that they are *delimiters* (ie that spaces separate items in a list rather than words in a sentence).
- Calling a variable with **single quotation marks** tells Bash to interpret everything literally. The fact that you are calling a variable is ignored!
...
Curly Brackets
--------------
Note that `$message` is actually shorthand for `${message}`: a dollar sign AND curly brackets are used to tell Bash what the name of your variable is, but, if you leave out the brackets then Bash can usually still guess correctly. However, this is not the case if you want your variable to appear in the middle of a string:
...
Built-In Variables
------------------
The 'hash', 'at' and 'question mark' symbols are all pre-defined variables in bash.
...
Data Types
==========
Unlike most programming languages, Bash doesn't worry too much about what *type* a particular variable is. If you want to treat a variable as a word you can, and if it contains only numbers and you want to treat it as a number you also can:
