---
title: '<font size="5">Introduction to Bash:</font><br>Booleans and If Statements'
output:
    html_document:
        theme: paper
        highlight: textmate
...
In Bash, another common way to work with binary true/false statements is by using "1" for true and "0" for false (although these are not proper Booleans):
...
Square brackets can be thought of as meaning 'test whether this statement is true'. They aren't necessary if the statement is literally the Boolean value 'true' or 'false' (as in the first two examples) because Bash has the answer already, but they are necessary if the statement is a number.
...
Whether or not something is 'not equal to' something else is tested with `!=`. In Bash, the exclamation mark means 'not' so `!=` means 'not equal'.
