...
- Close and re-open Sublime Text once it's finished
...
- **Terminus**
    ...
    - To create a key binding that opens a terminal pointing to the same folder that the active file is in:
        ...
        - This opens two files: the default key bindings on the left-hand side and the user's key bindings on the right-hand side. Confusingly, both of these files will be called "Default" - even the User key bindings file.
            - Specifically, both files will be called `Default (Linux).sublime-keymap` on Linux, `Default (OSX).sublime-keymap` on macOS and `Default (Windows).sublime-keymap` on Windows
...
- **Pretty JSON**
    ...
    - To use, open a JSON file and go to the Command Palette (Ctrl+Shift+P) > search for and run "Pretty JSON: Format JSON"
    - This command used to have a keybinding assigned to it automatically, but this is no longer the case. It can be added manually, however, as follows:
        - Go to "Key Bindings"
            - *Ubuntu:* Preferences > Key Bindings
            - *macOS:* Sublime Text > Preferences > Key Bindings
        - This opens two files: the default key bindings on the left-hand side and the user's key bindings on the right-hand side. Confusingly, both of these files will be called "Default" - even the User key bindings file.
            - Specifically, both files will be called `Default (Linux).sublime-keymap` on Linux, `Default (OSX).sublime-keymap` on macOS and `Default (Windows).sublime-keymap` on Windows
        - Add the following in the User key bindings file (on the right-hand side) anywhere between the starting and ending square brackets:
...
- **R-IDE**
    ...
    ```
    - If you want to render Python scripts with R Markdown you also need to have the "reticulate" package installed. Run the following in R to do this:
    ```{r, eval = FALSE}
...
+ **SublimeLinter**
    ...
    + Install the following linters (there's no harm in installing more rather than fewer):
        - Python:
            - Install these linters through pip (replace `3.11` with your version of Python):
                - `python3.11 -m pip install flake8`
                - `python3.11 -m pip install pycodestyle`
                - `python3.11 -m pip install pydocstyle`
                - `python3.11 -m pip install bandit`
                - `python3.11 -m pip install pyflakes`
...
- **PlainTasks**
    - TODO lists
    - Comes with a short tutorial: Preferences > Package Settings > PlainTasks > Tutorial
- **OmniMarkupPreviewer**
