...
**Troubleshooting: Cannot find a valid Python interpreter**
...
The fix is to set the Python path in the LaTeXTools package settings:

- Preferences > Package Settings > LaTeXTools > Settings
- In the **left-hand side** (usually changes to the settings are made in the right-hand side, but not in this case!) search for `"python": "",` which is nested under the `"linux"` section
- Add in the path to your Python installation:
...
The path to your Python installation can be found by running `which python3.13` or `which python3` from the terminal (depending on the version and instance of Python you want Sublime Text to use).
