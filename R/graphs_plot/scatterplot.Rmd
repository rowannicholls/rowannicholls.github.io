---
title: '<font size="5">Graphs in R using plot():</font><br>Scatter Plots'
output:
    html_document:
        theme: paper
        highlight: textmate
...
# Axis Options
The limits of the x- and y-axis can be changed with the `xlim` and `ylim` keyword arguments. Decide what you want the max and min values of the axes to be and specify these as vectors:
...
For a full list of what symbols can be used, click [here](scatterplot_symbols.html).
...
- Type out the colour's name, eg `col='blue'` will make the symbols blue. There are 657 colours that can be specified by name in this way, see them all [here](scatterplot_colours.html).
...
To add a 'line of best fit', use the `lm()` (<b>l</b>inear model) function then the `abline()` function (which plots a straight <b>line</b> between <b>a</b> and <b>b</b>.
