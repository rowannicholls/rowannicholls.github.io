...
spotify = data.frame(
    ...
    Gender=c('Male', 'Male', 'Male', 'Male', 'Male', 'Male', 'Female', 'Male', 'Male', 'Male'),
...
As you can see above, seven vectors were created using the `c()` function and each was given a name. This creates a data frame that looks like this:
```{r}
...
This is essentially the same as deleting columns, but this time you are choosing which columns to keep instead of which ones to get rid of.
```{r}
...
* The square brackets indicate that you are indexing (subsetting) the data frame
...
Looking up a value in a particular cell in the table is essentially a combination of the above: selecting a certain column and a certain row and seeing what is at their intersection.
```{r}
...
```{r}
df = spotify[!(rowSums(is.na(spotify['Featuring']))),]
print(df)
```
...
df = filter(spotify, Artist == 'Ed Sheeran' | Gender == 'Female')
df = filter(spotify, xor(Artist == 'Ed Sheeran', Streams.Millions >= 1500))
```

# Search

Searching is similar to filtering except instead of returning the rows that match a certain condition you return the locations of the matches.
```{r}
idx = match('Havana', spotify$'Song')
```

Sometimes you don't even want the locations of the value(s) you are looking for, you just want to know if they exist in the data frame or not. For example, is there a song by The Chainsmokers in the top 10?
```{r}
# Does a column contain a certain values?
bool = 'The Chainsmokers' %in% spotify$'Artist'
print(bool)
```

You can also search the column names to see if the one you are looking for has been created yet:
```{r}
...
* Data frames can be **indexed**, **cleaned**, **filtered** (aka subsetted), **searched**, **sorted**, etc
