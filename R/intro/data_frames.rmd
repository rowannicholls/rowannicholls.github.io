...
========================
If you know exactly which row(s) and column(s) you want, you can _index_ a data frame in order to _select_ only that portion of the whole object.
...
You can also search an entire column at once, ie search each row. For example, for each artist in the top 10, are they female?
```{r}
# Does the rows contain a certain value?
bool = (spotify$Gender == 'Female')
print(bool)
```
