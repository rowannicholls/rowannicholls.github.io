...
- Check that it's worked by opening the terminal and running `R --version`. This should show the start-up message (including the version number so you can double check that you've installed the latest stable release).
- Open R in the terminal by running `R`. This will open an interactive command line where you can enter and run R code manually. Close it by running `q()` and then `n` to not save the workspace image.
...
- Check that it's worked by opening the terminal and running `R --version`. This should show the start-up message (including the version number so you can double check that you've installed the latest stable release).
- Open R in the terminal by running `R`. This will open an interactive command line where you can enter and run R code manually. Close it by running `q()` and then `n` to not save the workspace image.
- You can download and install **RStudio** by going to [its website](https://rstudio.com/) > Download RStudio > under "RStudio Desktop" click "Download RStudio" > scroll down to "Ubuntu 22" (or "Ubuntu 18+" if you have an older version) and download its associated .deb file (`rstudio-2022.07.2-576-amd64.deb`).
...
such file or directory
    ```
    It means you need to install PostgreSQL; follow the instructions [here](https://www.postgresql.org/download/linux/ubuntu/) although running `sudo apt-get install postgresql-12` is a good start.
