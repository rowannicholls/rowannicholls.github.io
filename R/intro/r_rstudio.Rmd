...
- First, install R [from CRAN](https://cran.rstudio.com/) (the Comprehensive R Archive Network)
    - On Ubuntu, an alternative is to install it from the terminal with:
    ```{bash, eval = FALSE}
...
sudo apt install r-base
    ```
    - Check that it's worked by opening the terminal/command prompt and running `R --version`. This should show the start-up message (including the version number so you can double check that you've installed the latest stable release).
...
- Then, install RStudio: go to [the RStudio website](https://rstudio.com/) > Download > download RStudio Desktop
    - On macOS, drag the RStudio icon to the Applications folder so it can be accessed easily
    - On Ubuntu, download the .deb installer rather than the .tar.gz one (unless you have a particular reason for wanting it)
