...
**However**, the above code is *not good practice*. Instead of iterating over the numbers from 1 to the number of rows in the data frame (`nrow()`), as was done above, you should instead use `seq_len(nrow())` like so:
