...
To update R and RStudio manually, download and install the latest version from [the R website](https://www.r-project.org/). When you re-start RStudio it should automatically use this latest version. Note that if you are moving to a new major version (eg from version 3.5 to version 4.0) then you will have to re-install all your packages. Trying to simply update them will not work; the easiest thing to do is to delete all of them completely and then re-install from scratch. To find where they are installed on your computer, run `.libPaths()` in R and open the folder that is returned. Delete all the folders in there.
