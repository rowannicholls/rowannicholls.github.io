...
Don't underestimate the power of comments! Just because your code makes sense to you now when you're writing it doesn't mean you will remember what you were thinking 6 months down the line when you re-open it. Likewise, if someone else needs to use your code they will need to work out what it does; and almost impossible task if you don't explain yourself!
...
Notice that when you assign a number to a variable it no longer gets outputted to the console. Variable assignment __*suppresses*__ output!
...
Here's something that often confuses beginner coders: the statement `x = x + 1`. In mathematics, it doesn't make any sense! There's no value of `x` that will satisfy the equation! However, in programming, this statement is not an equation but an __*assignment*__: you are giving a variable `x` a value that is equal to that of an existing variable, plus one:
...
Often, instead of using the equals sign, R programmers will use a left-arrow when assigning values to variables (`<-`). This helps them avoid the confusion described above.

```{r}
# The left-arrow is the same as a single equals sign in R
x <- 6
# You may find this a less confusing notation
x <- x + 1
...
These are GOOD examples of variable names:
...
# Nulls
Nulls are, as the name suggests, equal to nothing. While it may seem weird that you can set a variable to nothing these become useful when you're dealing with missing data.
