...
Remember, a single equals sign is used to assign a value to a variable. So you can test equality using `==`, generate a Boolean, then assign this Boolean to a variable using `=`:
```{r}
# Is 1 equal to 1?
bool = (1 == 1)
print(bool)
...
You can also test if a string contains a certain sub-string using `grepl()`:
```{r}
st = 'Hello World'
...
```{r}
if (1 == 1) {
	print("This will print to console because 1 is equal to 1 and so '1 == 1' evaluates to TRUE")
}
```
...
if (age <= 110) {
	...
	stop('This person is older than 110 years. Are you sure their age is correct?')
}
```
