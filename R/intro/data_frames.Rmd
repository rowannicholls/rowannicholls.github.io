...
--------------------------
Given the pre-installed `iris` data frame:

```{r}
head(iris)
```

Sum all the values in the columns *without grouping* by selecting the numeric columns and using `colSums()` to add up all the values in each:
...
Sum all the values in the columns *with grouping* by selecting the numeric columns, specifying the groups in the data and using `rowsum()` to add up all the values in each numeric column for each group. In this example, the groups are the `Species` names:
...
-----------------------
Given the pre-installed `randu` data frame:

```{r}
head(randu)
```

Sum all the values in each row using the `rowSums()` function:
