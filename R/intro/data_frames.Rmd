...
# Delete multiple columns by listing their names using the select() function from the dplyr package
library(dplyr)
...
bool = 'Weeks.at.no1' %in% colnames(spotify)
print(bool)
```
...
```{r}
df = subset(spotify, Streams.Millions >= 1700)
proportion = nrow(df) / nrow(spotify)
percentage = nrow(df) / nrow(spotify) * 100
printf('Number of songs with over 1 700 million streams: %s (%3.1f or %3.1f%%)', nrow(df), proportion, percentage)
```

Note that the `printf()` function that was used above comes from the R.utils library.
