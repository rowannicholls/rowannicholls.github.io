...
In R, all three are performed using the same function (`chisq.test()`) from the base package. For more info about this function, see https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/chisq.test
...
```{r, eval=FALSE}
# THE R.utils LIBRARY IS NEEDED FOR printf()
library(R.utils)

result = chisq.test(c(44, 56), p=c(1/2, 1/2))
printf('p = %.2f', round(result$p.value, 2))
```
...
printf('p = %.1f', round(result$p.value, 1))
```
...
ncols = 4
df = (nrows - 1) * (ncols - 1)
printf('Degrees of freedom = %d', df)
```
...
printf('chi-square = %.2f', round(chisq, 2))
```
...
result = chisq.test(df)
printf('p = %.4f', round(result$p.value, 4))
```
...
result = chisq.test(df)
printf('p = %.3f', round(result$p.value, 3))
```
