---
title: '<font size="5">Statistics in R:</font><br>Diagnostic Accuracy Tutorial'
output:
    html_document:
        theme: paper
        highlight: textmate
...
to_print = titanic_train[20:25, ]
rownames(to_print) = NULL
print(kable(to_print[, -match(c('SibSp', 'Parch'), names(to_print))]))
```
...
```{r, echo=FALSE}
options(width=200)
to_print = subset(df, Pclass == '3')[33:38, ]
rownames(to_print) = NULL
print(to_print[, -match(c('Parch'), names(to_print))])
```
...
```{r, echo=FALSE}
options(width=200)
to_print = subset(df, Age >= 20 & Age < 30)[16:21, ]
rownames(to_print) = NULL
print(to_print[, -match(c('Parch'), names(to_print))])
```
