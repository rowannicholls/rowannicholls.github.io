...
will plot a dashed line. Note that the `type` keyword argument needs to be set to `l` for a line to be plotted (only the points will be plotted by default - ie a scatter plot instead of a line plot). The lines types that can be plotted are as follows:
