---
title: '<font size="5">Graphs in R using ggplot2:</font><br>Box Plots (aka Box-and-Whisker Plots)'
output:
    html_document:
        theme: paper
        highlight: textmate
...
-----------------------------
This example uses the `chickwts` dataset; a record of the weights of 71 chicks which had been fed one of 6 different diets:
...
p = p + geom_boxplot()
p = p + scale_fill_manual(values=c('cadetblue2', 'tomato', 'khaki1', 'darkseagreen3'))
p = p + labs(title="Men's Long Jump Finals", x='', y='Distance [m]', fill='Olympic Games')
print(p)
```
...
sample_size = function(x) {
    # SAMPLE SIZE LABELS
    return(data.frame(y=median(x) * 1.003, label=paste0('n = ', length(x))))
}
...
p = p + geom_boxplot()
p = p + scale_fill_manual(values=c('cadetblue2', 'tomato', 'khaki1', 'darkseagreen3'))
p = p + labs(title="Women's Long Jump Finals", x='', y='Distance [m]', fill='Olympic Games')
p = p + stat_summary(fun.data=sample_size, geom='text', fun=median)
p = p + stat_summary(fun.data=sample_mean, geom='text', fun=mean, colour='black')
print(p)
```
...
- `png('Name of Plot.png')`
- `pdf('Name of Plot.pdf')`

Regardless of which of the above you choose, it must come __*before*__ you start plotting the graph (ie before you call `ggplot()`).
